  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${log.title}"` : 'Creating a Memory'}</Typography>
        ...
        <TextField name="entry" variant="outlined" label="Entry" fullWidth multiline rows={4} value={logData.entry} onChange={(e) => setLogData({ ...logData, entry: e.target.value })} />
